...
SESSIONS_DIR = None
USER_NAME = None
WORKSPACE_REGISTRY = None  # WorkspaceRegistry for multi-agent support
...
def detect_config(args=None):
    ...
    global WORKSPACE, MEMORY_DIR, LOG_DIR, SESSIONS_DIR, USER_NAME, WORKSPACE_REGISTRY

    # --workspace is now a list (action='append'); extract first for backward compat
    workspace_list = []
    if args and args.workspace:
        workspace_list = list(args.workspace) if isinstance(args.workspace, list) else [args.workspace]
    ...
    if data_dir and os.path.isdir(data_dir):
        ...
        if os.path.isdir(ws) and not workspace_list:
            workspace_list = [ws]
    ...
    # 1. Workspace — primary workspace for backward compat (first from list, env, or auto-detect)
    primary_workspace = workspace_list[0] if workspace_list else None
    if primary_workspace:
        WORKSPACE = os.path.expanduser(primary_workspace)
    ...
    # 5. Multi-agent workspace discovery
    multi_agent = getattr(args, 'multi_agent', True) if args else True
    if multi_agent:
        try:
            _discover_workspaces(data_dir or "", workspace_list)
        except Exception:
            pass

    # 6. Initialize DataProvider (multi or single)
    try:
        _init_data_provider()
    except Exception:
        pass
...
def _discover_workspaces(data_dir: str, extra_workspaces: list) -> None:
    """Populate the global WORKSPACE_REGISTRY via tiered discovery."""
    global WORKSPACE_REGISTRY
    try:
        from clawmetry.workspaces import WorkspaceRegistry
    except ImportError:
        return

    registry = WorkspaceRegistry()

    env_workspaces = os.environ.get("CLAWMETRY_WORKSPACES", "")
    if env_workspaces:
        extra_workspaces = list(extra_workspaces) + [
            p.strip() for p in env_workspaces.split(":") if p.strip()
        ]

    gw_invoke_fn = None
    try:
        gw_invoke_fn = _gw_invoke
    except NameError:
        pass

    registry.discover(
        data_dir=data_dir,
        gw_invoke=gw_invoke_fn,
        extra_workspaces=extra_workspaces if len(extra_workspaces) > 1 else None,
    )
    ...
    if registry.count() > 0:
        ...
        if default:
            global WORKSPACE, MEMORY_DIR, SESSIONS_DIR
            WORKSPACE = default.workspace
            MEMORY_DIR = os.path.join(default.workspace, "memory") if default.workspace else MEMORY_DIR
            if default.sessions_dir:
                SESSIONS_DIR = default.sessions_dir
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; transition: background-color 0.3s ease, color 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease; }
  ...
  .badge.agent { background: var(--bg-tertiary); color: var(--text-accent); font-size: 10px; padding: 1px 6px; border-radius: 4px; margin-right: 4px; }
  .agent-pill { display: inline-flex; align-items: center; gap: 4px; padding: 4px 12px; border-radius: 16px; font-size: 12px; font-weight: 500; color: var(--text-secondary); background: var(--bg-tertiary); border: 1px solid var(--border-primary); cursor: pointer; transition: all 0.15s; user-select: none; }
  .agent-pill:hover { background: var(--bg-hover); color: var(--text-primary); border-color: var(--text-muted); }
  .agent-pill.active { background: var(--bg-accent); color: #fff; border-color: var(--bg-accent); }
...
<div id="agent-bar" style="display:none;padding:6px 16px;background:var(--bg-secondary);border-bottom:1px solid var(--border-primary);overflow-x:auto;white-space:nowrap;">
  <div id="agent-pills" style="display:inline-flex;gap:6px;align-items:center;"></div>
</div>
...
var _isMultiAgent = false;
var _currentTab = 'overview';

function agentParam() {
  if (!_selectedAgent || !_isMultiAgent) return '';
  return 'agent=' + encodeURIComponent(_selectedAgent);
}
...
function agentLabel(agentId) {
  var a = _agents.find(function(x) { return x.id === agentId; });
  return a ? ((a.emoji || '') + ' ' + (a.label || a.id)).trim() : (agentId || '');
}
...
function agentBadge(agentId) {
  if (!_isMultiAgent || _selectedAgent) return '';
  if (!agentId) return '';
  return '<span class="badge agent">' + escHtml(agentLabel(agentId)) + '</span>';
}

function renderAgentBar(agents) {
  var bar = document.getElementById('agent-bar');
  var pills = document.getElementById('agent-pills');
  if (!bar || !pills) return;
  var html = '<span class="agent-pill' + (!_selectedAgent ? ' active' : '') + '" onclick="setAgent(null)">All Agents</span>';
  agents.forEach(function(a) {
    var isActive = _selectedAgent === a.id;
    var safeId = escHtml(a.id).replace(/'/g, "\\'");
    html += '<span class="agent-pill' + (isActive ? ' active' : '') + '" onclick="setAgent(\'' + safeId + '\')">';
    html += (a.emoji ? a.emoji + ' ' : '') + escHtml(a.label || a.id);
    html += '</span>';
  });
...
function setAgent(agentId) {
  ...
  try { localStorage.setItem('clawmetry-agent', agentId || ''); } catch(e) {}
  if (_isMultiAgent) renderAgentBar(_agents);
...
async function loadAll() {
  try {
    ...
    var overview = await fetchJsonWithTimeout(apiUrl('/api/overview'), 3000);
    ...
    try {
      var usage = await fetchJsonWithTimeout(apiUrl('/api/usage'), 5000);
...
async function loadMiniWidgets(overview, usage) {
  ...
  fetch(apiUrl('/api/sessions')).then(function(r){return r.json()}).then(function(sd) {
...
async function loadActivityStream() {
  try {
    var transcripts = await fetchJsonWithTimeout(apiUrl('/api/transcripts'), 4000);
    var activities = [];
    ...
    if (transcripts.transcripts && transcripts.transcripts.length > 0) {
      ...
      try {
        var transcript = await fetchJsonWithTimeout(apiUrl('/api/transcript/' + recent.id), 4000);
...
function showSessionsModal() {
  fetch(apiUrl('/api/sessions')).then(r=>r.json()).then(function(d) {
...
async function viewFile(path) {
  ...
  try {
    var data = await fetch(apiUrl('/api/file?path=' + encodeURIComponent(path))).then(r => r.json());
...
async function loadSessions() {
  ...
  var [sessData, saData] = await Promise.all([
    fetch(apiUrl('/api/sessions')).then(r => r.json()).catch(function() { return {sessions:[]}; }),
  ...
  mainSessions.forEach(function(s) {
    ...
    html += '<div class="session-name">' + agentBadge(s.agent_id) + '🖥️ ' + escHtml(s.displayName || s.key) + ' <span style="font-size:11px;color:var(--text-muted);font-weight:400;">Main Session</span></div>';
...
async function loadMemory() {
  ...
  loadMemoryAnalytics();
  var data = await fetch(apiUrl('/api/memory-files')).then(r => r.json());
  var el = document.getElementById('memory-list');
  // Add hover CSS once
  ...
  // In multi-agent "All" mode, group by agent first
  var agentGroups = {};
  if (_isMultiAgent && !_selectedAgent) {
    data.forEach(function(f) {
      var aid = f.agent_id || 'default';
      if (!agentGroups[aid]) agentGroups[aid] = [];
      agentGroups[aid].push(f);
    });
  }
  var roots = [], folders = {};
  data.forEach(function(f) {
  ...
  var html = '';
  if (_isMultiAgent && !_selectedAgent && Object.keys(agentGroups).length > 1) {
    Object.keys(agentGroups).sort().forEach(function(aid) {
      html += '<div style="padding:10px 14px 4px;font-size:11px;font-weight:700;color:var(--text-accent);border-top:1px solid var(--border-primary);margin-top:4px;">' + escHtml(agentLabel(aid)) + '</div>';
      agentGroups[aid].forEach(function(f) {
        var icon = f.path.endsWith('.md') ? '📝' : '📄';
        var sz = f.size > 1024 ? (f.size/1024).toFixed(1) + 'K' : f.size + 'B';
        html += '<div class="mem-file" data-path="' + escHtml(f.path) + '" data-agent="' + escHtml(f.agent_id || '') + '" style="display:flex;align-items:center;gap:6px;padding:5px 14px 5px 28px;cursor:pointer;font-size:12px;color:var(--text-secondary)">' +
  ...
  roots.forEach(function(f) {
    var icon = f.path.endsWith('.md') ? '📝' : '📄';
    var sz = f.size > 1024 ? (f.size/1024).toFixed(1) + 'K' : f.size + 'B';
    html += '<div class="mem-file" data-path="' + escHtml(f.path) + '" data-agent="' + escHtml(f.agent_id || '') + '" style="display:flex;align-items:center;gap:6px;padding:5px 14px;cursor:pointer;font-size:12px;color:var(--text-secondary)">' +
      '<span style="flex-shrink:0">' + icon + '</span>' +
      '<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escHtml(f.path) + '</span>' +
      '<span style="color:var(--text-muted);font-size:10px;flex-shrink:0">' + sz + '</span></div>';
  });
  ...
  } // end else (single-agent file grouping)
  ...
  sidebar.querySelectorAll('.mem-file').forEach(function(row) {
    row.onclick = async function() {
      ...
      var fileAgent = this.dataset.agent;
      ...
      try {
        var fileUrl = '/api/file?path=' + encodeURIComponent(p); if (fileAgent && !_selectedAgent) fileUrl += '&agent=' + encodeURIComponent(fileAgent);
        var d = await fetch(apiUrl(fileUrl)).then(function(r) { return r.json(); });
...
async function loadUsage() {
  try {
    var data = await fetch(apiUrl('/api/usage')).then(r => r.json());
    function fmtTokens(n) { return n >= 1000000 ? (n/1000000).toFixed(1) + 'M' : n >= 1000 ? (n/1000).toFixed(0) + 'K' : String(n); }
    function fmtCost(c) { return c >= 0.01 ? '$' + c.toFixed(2) : c > 0 ? '<$0.01' : '$0.00'; }
...
async function loadTranscripts() {
  try {
    var data = await fetch(apiUrl('/api/transcripts')).then(r => r.json());
    var html = '';
    data.transcripts.forEach(function(t) {
      html += '<div class="transcript-item" onclick="viewTranscript(\'' + escHtml(t.id) + '\')">';
      html += '<div><div class="transcript-name">' + agentBadge(t.agent_id) + escHtml(stripNsId(t.name)) + '</div>';
...
async function viewTranscript(sessionId) {
  ...
  try {
    var data = await fetch(apiUrl('/api/transcript/' + encodeURIComponent(sessionId))).then(r => r.json());
    // Metadata
...
function initFlow() {
  ...
  fetch(apiUrl('/api/overview')).then(function(r){return r.json();}).then(async function(d) {
...
function updateFlowStats() {
  ...
  if (flowStats.events % 15 === 0) {
    fetch(apiUrl('/api/overview')).then(function(r){return r.json();}).then(function(d) {
      var tok = document.getElementById('flow-tokens');
      if (tok) tok.textContent = (d.mainTokens / 1000).toFixed(0) + 'K';
    }).catch(function(){});
  }
...
async function bootDashboard() {
  ...
  // Discover agents for multi-workspace support
  try {
    var agentData = await fetch('/api/agents').then(function(r) { return r.json(); });
    _agents = agentData.agents || [];
    _isMultiAgent = agentData.multi || false;
    if (_isMultiAgent) {
      var saved = '';
      try { saved = localStorage.getItem('clawmetry-agent') || ''; } catch(e) {}
      if (saved && _agents.some(function(a) { return a.id === saved; })) {
        _selectedAgent = saved;
      }
      renderAgentBar(_agents);
    }
  } catch(e) {
    console.warn('Agent discovery unavailable', e);
  }
...
# ── Multi-agent route helpers ─────────────────────────────────────────────────

def _get_agent_filter():
    """Extract ?agent= query param. Returns agent_id or None for 'all'."""
    agent = request.args.get('agent', '').strip()
    return agent if agent and agent != 'all' else None


def _get_multi_provider():
    """Return the MultiWorkspaceProvider if active, else None."""
    try:
        from clawmetry.providers import get_active_provider
        from clawmetry.providers.multi import MultiWorkspaceProvider
        provider = get_active_provider()
        if isinstance(provider, MultiWorkspaceProvider):
            return provider
    except ImportError:
        pass
    return None
...
@bp_overview.route('/api/agents')
def api_agents():
    """Return discovered agent workspaces for multi-agent dashboard."""
    if WORKSPACE_REGISTRY and WORKSPACE_REGISTRY.count() > 0:
        return jsonify({
            'agents': WORKSPACE_REGISTRY.to_list(),
            'multi': WORKSPACE_REGISTRY.is_multi(),
            'discoverySource': WORKSPACE_REGISTRY.discovery_source,
        })
    return jsonify({
        'agents': [{
            'id': 'main',
            ...
            'label': 'main',
            'isDefault': True,
            'workspaceExists': os.path.isdir(WORKSPACE) if WORKSPACE else False,
            'sessionsDirExists': os.path.isdir(SESSIONS_DIR) if SESSIONS_DIR else False,
        }],
        'multi': False,
...
@bp_overview.route('/api/agents/refresh', methods=['POST'])
def api_agents_refresh():
    """Re-run agent workspace discovery."""
    data_dir = _auto_detect_data_dir() or ''
    if WORKSPACE_REGISTRY:
        WORKSPACE_REGISTRY.clear()
        WORKSPACE_REGISTRY.discover(data_dir=data_dir, gw_invoke=_gw_invoke)
        return jsonify({
            'agents': WORKSPACE_REGISTRY.to_list(),
            'multi': WORKSPACE_REGISTRY.is_multi(),
            'discoverySource': WORKSPACE_REGISTRY.discovery_source,
        })
    return jsonify({'agents': [], 'multi': False, 'discoverySource': 'none'})


@bp_overview.route('/api/overview')
def api_overview():
    multi = _get_multi_provider()
    agent = _get_agent_filter()

    if multi and (agent or multi):
        provider_sessions = multi.list_sessions(limit=50, agent_id=agent)
        sessions = [
            {'sessionId': s.session_id, 'key': s.session_id[:12] + '...',
             'displayName': s.display_name, 'model': s.model,
             'updatedAt': s.updated_at, 'totalTokens': s.total_tokens,
             'channel': s.extra.get('channel', 'unknown'),
             'contextTokens': s.extra.get('contextTokens', 200000),
             'kind': s.kind, 'agent_id': s.agent_id}
            for s in provider_sessions
        ]
        mem_files_raw = multi.list_memory_files(agent_id=agent)
        mem_files = [{'path': mf.path, 'size': mf.size} for mf in mem_files_raw]
    else:
...
def api_sessions():
    ...
    if multi:
        sessions = [
            {**s.extra, 'sessionId': s.session_id, 'agent_id': s.agent_id,
             'displayName': s.display_name, 'model': s.model,
             'updatedAt': s.updated_at, 'totalTokens': s.total_tokens}
            for s in multi.list_sessions(limit=50, agent_id=agent)
        ]
        return jsonify({'sessions': sessions, 'multi': True})
...
def api_memory_files():
    ...
    agent = _get_agent_filter()
    if multi:
        files = multi.list_memory_files(agent_id=agent)
        return jsonify([
            {'path': mf.path, 'size': mf.size, 'modified': mf.modified, 'agent_id': mf.agent_id}
            for mf in files
        ])
...
def api_view_file():
    ...
    multi = _get_multi_provider()
    if multi and agent:
        content = multi.read_workspace_file(path, agent_id=agent)
        if content:
            return jsonify({'path': path, 'content': content[:100_000], 'agent_id': agent})
...
def api_usage():
    ...
    cache_key = agent or '__all__'
    if not agent and _usage_cache['data'] is not None and (now - _usage_cache['ts']) < _USAGE_CACHE_TTL:
        return jsonify(_usage_cache['data'])

    if not agent and _has_otel_data():
    ...
    sessions_dirs = []
    if multi and agent:
        ws = WORKSPACE_REGISTRY.get(agent) if WORKSPACE_REGISTRY else None
        if ws and ws.sessions_dir and os.path.isdir(ws.sessions_dir):
            sessions_dirs = [ws.sessions_dir]
    elif multi and not agent:
        for ws in (WORKSPACE_REGISTRY.list_all() if WORKSPACE_REGISTRY else []):
            if ws.sessions_dir and os.path.isdir(ws.sessions_dir):
                sessions_dirs.append(ws.sessions_dir)
    else:
        sd = SESSIONS_DIR or os.path.expanduser('~/.openclaw/agents/main/sessions')
        if os.path.isdir(sd):
            sessions_dirs = [sd]
...
def api_transcripts():
    ...
    if multi:
        all_transcripts = []
        for aid, provider in multi._iter_providers(agent):
            sd = provider.sessions_dir
            if not sd or not os.path.isdir(sd):
                continue
            for fname in sorted(os.listdir(sd), key=lambda f: os.path.getmtime(os.path.join(sd, f)), reverse=True):
                if not fname.endswith('.jsonl') or 'deleted' in fname:
                    continue
                fpath = os.path.join(sd, fname)
                try:
                    msg_count = sum(1 for _ in open(fpath))
                    raw_id = fname.replace('.jsonl', '')
                    ...
                    pass
        all_transcripts.sort(key=lambda t: t.get('modified', 0), reverse=True)
        return jsonify({'transcripts': all_transcripts[:50], 'multi': True})
...
def api_transcript(session_id):
    ...
    multi = _get_multi_provider()
    if multi and ':' in session_id:
        from clawmetry.providers.multi import _parse_ns_session_id
        agent_id, original_id = _parse_ns_session_id(session_id)
        provider = multi.provider_for(agent_id)
        if provider and provider.sessions_dir:
            sessions_dir = provider.sessions_dir
            fpath = os.path.join(sessions_dir, original_id + '.jsonl')
...
def _run_server(args):
    ...
    try:
        ...
        if WORKSPACE_REGISTRY and WORKSPACE_REGISTRY.is_multi():
            print(f"  Agents:     {WORKSPACE_REGISTRY.count()} discovered (via {WORKSPACE_REGISTRY.discovery_source})")
            for ws in WORKSPACE_REGISTRY.list_all():
                marker = " (default)" if ws.is_default else ""
                emoji = f" {ws.emoji}" if ws.emoji else ""
                print(f"    - {ws.label or ws.agent_id}{emoji}{marker}")
                print(f"      workspace: {ws.workspace}")
                if ws.sessions_dir:
                    print(f"      sessions:  {ws.sessions_dir}")
        else:
...
def _init_data_provider():
    """Initialize the active DataProvider after path detection.

    Uses MultiWorkspaceProvider when multiple agent workspaces are discovered,
    otherwise falls back to the standard single-workspace provider.
    """
    try:
        from clawmetry.providers import init_providers, set_active_provider
...
def main():
    ...
    shared.add_argument('--workspace', '-w', type=str, action='append', default=None,
                        help='Agent workspace directory (repeatable for multi-agent)')
    shared.add_argument('--data-dir', '-d', type=str, help='OpenClaw data directory (e.g. ~/.openclaw).')
    shared.add_argument('--multi-agent', action='store_true', default=True,
                        help='Enable multi-agent workspace discovery (default: True)')
    shared.add_argument('--single-agent', dest='multi_agent', action='store_false',
                        help='Disable multi-agent discovery (single workspace only)')
